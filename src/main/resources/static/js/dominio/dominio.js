...
$('#disable-modal').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget) // Button that triggered the modal
    var recipient_value = button.data('value') // Extract info from data-* attributes
    var recipient_id = button.data('id')
    // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
    // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
    var modal = $(this)
    var modal_title = modal.find('.modal-title');
    modal_title.text('Desactivar valor del dominio ' + recipient_value)
    var a = modal.find('.modal-body a')[0];
...
$('#enable-modal').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    var modal_title = modal.find('.modal-title');
    modal_title.text('Activar valor del dominio ' + recipient_value)
    var a = modal.find('.modal-body a')[0];
